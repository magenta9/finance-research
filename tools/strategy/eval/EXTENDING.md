...
defaultAllocationStrategyRegistry   # QuantDesk：策略算法唯一来源
...
- **策略逻辑** 只加在 QuantDesk `defaultAllocationStrategyRegistry`，eval 不复制算法。
...
### 1. 在 QuantDesk 注册策略（必须）

在 [`tools/desktop/quantdesk/packages/main/src/portfolio/strategy-registry.ts`](../../desktop/quantdesk/packages/main/src/portfolio/strategy-registry.ts) 中：
...
4. 补充 `packages/main/src/portfolio/pipeline.test.ts` 或 handler 单测。

Eval **不会**自动发现未注册的策略；registry 是 desktop 与 eval 的共享策略源。
...
1. QuantDesk registry 注册 handler  
...
| QuantDesk registry | [`strategy-registry.ts`](../../desktop/quantdesk/packages/main/src/portfolio/strategy-registry.ts) |
