...
export const denoiseCovarianceMarchenkoPastur = (
    ...
    const noiseValues = eigenvalues.filter((value: number) => value <= maxEigenvalue);
    const noiseAverage = noiseValues.length > 0
        ? noiseValues.reduce((sum: number, value: number) => sum + value, 0) / noiseValues.length
        : 0;
    const denoisedEigenvalues = eigenvalues.map((value: number) => (
