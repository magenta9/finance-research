...
面向多个 Allocation Strategy 的离线评价框架，用 Desktop Tool 内置资产池作为候选标的来源，并支持按市场筛选候选范围；它不是 Active Dual Momentum GTAA 的专属机制评测。
...
Strategy Eval Framework 的首个候选资产范围，由 Desktop Asset Universe Snapshot 中 market 为 A 和 BOND 的标的组成，用于随机抽取 10、15、20 个标的的组合。
...
Strategy Eval Framework 对一组 case 分数的汇总得分，首版公式为 `0.5 * p50Score + 0.25 * p10Score + 0.25 * p90Score`，其中 p90Score 表示 90% 分位值而不是最高 10% 样本均值。
...
Strategy Eval Framework 对单个回测 case 的分数，沿用当前 Active Dual Momentum Eval 的指标公式：Sharpe 50%、最大回撤 30%、波动 20%，其中 Sharpe ceiling 为 2.0。
