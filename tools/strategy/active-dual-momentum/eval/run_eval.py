...
CANDIDATE_SPACE_PATH = SCRIPT_DIR / "config/candidate-space.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Generate and evaluate N random ADM candidate configs in addition to baseline.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> int:
    ...
    strategy_configs = build_strategy_configs(
        strategy_config, args.optimize_iterations, args.seed
    )
    warmup_days = (
        max(
    ...
    strategy_summaries = summarize_by_strategy(rows)
    write_json(run_dir / "baseline-results.json", rows)
    write_json(run_dir / "score-summary.json", summary)
    write_json(run_dir / "strategy-summary.json", strategy_summaries)
...
def build_strategy_configs(
    base_config: dict[str, Any], optimize_iterations: int, seed: int
) -> list[dict[str, Any]]:
    configs = [{"strategyId": "baseline", "config": base_config}]
    if optimize_iterations <= 0:
        return configs

    candidate_space = load_json(CANDIDATE_SPACE_PATH)["parameters"]
    rng = random.Random(seed + 99_001)
    seen = {config_hash(base_config)}
    attempts = 0
    while (
        len(configs) < optimize_iterations + 1 and attempts < optimize_iterations * 100
    ):
        attempts += 1
        candidate = {
            "absoluteMomentumFilter": rng.choice(
                candidate_space["absoluteMomentumFilter"]
            ),
            "longLookbackWeeks": rng.choice(candidate_space["longLookbackWeeks"]),
            "shortLookbackWeeks": rng.choice(candidate_space["shortLookbackWeeks"]),
            "sleeveWeights": rng.choice(candidate_space["sleeveWeights"]),
            "topK": rng.choice(candidate_space["topK"]),
            "transactionCostBps": rng.choice(candidate_space["transactionCostBps"]),
            "slippageBps": rng.choice(candidate_space["slippageBps"]),
        }
        digest = config_hash(candidate)
        if digest in seen:
            continue
        seen.add(digest)
        configs.append(
            {
                "strategyId": f"candidate-{len(configs):03d}-{digest[:8]}",
                "config": candidate,
            }
        )
    return configs
...
def summarize_by_strategy(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    grouped: dict[str, list[dict[str, Any]]] = {}
    for row in rows:
        grouped.setdefault(row.get("strategyId", "baseline"), []).append(row)

    summaries = []
    for strategy_id, strategy_rows in grouped.items():
        summary = summarize_scores(strategy_rows)
        summaries.append({"strategyId": strategy_id, **summary})
    return sorted(
        summaries,
        key=lambda row: row["meanScore"] if row["meanScore"] is not None else -1,
        reverse=True,
    )
